...
for(int j = PrimeNumGen.Primes[65]; j < PrimeNumGen.Primes[66]; j++)
{
    int res =PrimeNumGen.BinarySearch(j);
    if (res != 66)
    {
        Console.WriteLine(j);
    }
...
//d.Remove(4);
//int var = 5;
//Console.WriteLine(EqualityComparer<int>.Default.GetHashCode(5));
//Console.WriteLine(var.GetHashCode());
